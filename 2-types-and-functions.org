* Chương 2: Kiểu và hàm

** Kiểu dữ liệu có gì quan trọng?

Trong Haskell, mỗi biểu thức và hàm đều có /kiểu/ (dữ liệu).
Chẳng hạn, giá trị ~True~ có kiểu ~Bool~, còn giá trị
~"foo"~ có kiểu String. Kiểu của một giá trị chỉ ra rằng
giá trị đó chung các thuộc tính nhất định với những giá trị khác giống kiểu.
Chẳng hạn, ta có thể cộng các số, và có thể nối các danh sách; đây 
là những thuộc tính của các kiểu nêu trên. Ta nói một biểu thức 
"có kiểu ~X~", hay "thuộc kiểu ~X~".

Trước khi đi sâu vào thảo luận hệ thống kiểu của Haskell,
hãy cùng nói về lí do tại sao ta lại cần phải để tâm đến các kiểu:
chúng /để làm gi/? Ở cấp độ thấp nhất, máy tính chỉ
quan tâm tới các byte, và hầu như chẳng có cấu trúc nào nữa. 
Một hệ thống kiểu cho ta cái gọi là /trừu tượng hoá/. Một kiểu sẽ 
thêm ý nghĩa vào các byte thuần tuý: kiểu cho phép ta nói
"các bytes này là dòng chữ", "các byte nọ là 
bản đặt chỗ hàng không", v.v. Thông thường, một hệ thống kiểu 
còn đi xa hơn nữa và giúp ta tránh việc vô ý làm đảo lộn các kiểu: 
chẳng hạn, một hệ thống kiểu thường sẽ không cho phép ta coi 
bản đặt chỗ khách sạn như là một biên lai thuê xe.

Lợi ích từ việc giới thiệu trừu tượng hoá là nó cho phép ta quên đi
hoặc phớt lờ những chi tiết ở cấp độ thấp. Nếu tôi biết rằng một giá trị
trong chương trình là kiểu chuỗi thì tôi không cần biết chi li 
các chuỗi được thực hiện ra sao: tôi cứ việc coi như chuỗi sẽ 
ứng xử giống tất thảy các chuỗi khác mà tôi từng thao tác.

Điều khiến cho hệ thống kiểu thú vị là ở chỗ không phải chúng đều như nhau.
Sự thực là các hệ thống kiểu khác nhau thường không để tâm đến cùng 
một kiểu bài toán. Hệ thống kiểu trong một ngôn ngữ lập trình nào
thì khắc hoạ sâu sắc cách ta suy nghĩ và viết mã lệnh bằng ngôn ngữ đó.

Hệ thống kiểu Haskell cho phép ta nghĩ ở cấp độ rất trừu tượng: 
nó cho phép ta viết các chương trình gọn gàng, mạnh mẽ.

** Hệ thống kiểu Haskell

Các kiểu trong Haskell có ba điểm thú vị: chúng là kiểu
/mạnh/, /tĩnh/, và có thể tự động /suy luận/ được. Hãy 
cùng nhau nói thêm về từng khái niệm này.
Mỗi khi có thể, chúng tôi sẽ trình bày sự giống nhau giữa
các khái niệm hệ thống kiểu Haskell và các khái niệm liên quan
trong những ngôn ngữ khác. Chúng tôi cũng sẽ đề cập tới các
ưu điểm và nhược điểm của từng thuộc tính này.

*** Kiểu mạnh

Khi nói Haskell có hệ thống kiểu /mạnh/, chúng tôi ngụ ý rằng
hệ thống kiểu đảm bảo cho chương trình không thể chứa những 
loại lỗi nhất định được. Những lỗi như vậy bắt nguồn từ việc
viết các biểu thức vô nghĩa, như dùng một số nguyên làm
mọt hàm. Chẳng hạn, nếu bạn trông đợi một hàm làm việc với
số nguyên nhưng rồi lại truyền cho nó một chuỗi thì trình biên dịch
Haskell sẽ từ chối việc này.

Ta gọi biểu thức tuân thủ theo quy tắc kiểu của ngôn ngữ là 
biểu thức /đúng kiểu/. Một biểu thức vi phạm quy tắc về kiểu
thì gọi là /sai kiểu/, và sẽ gây nên một /lỗi về kiểu/.

Một khía cạnh khác trong quan điểm của Haskell về định kiểu mạnh,
đó là nó sẽ không tự động quy đổi giá trị qua lại giữa các kiểu được.
Chẳng hạn, một trình biên dịch C sẽ lẳng lặng tự động quy đổi 
giúp ta một giá trị nguyên sang kiểu phẩy động nếu một hàm trông đợi 
tham số phải có kiểu phẩy động, nhưng trình biên dịch Haskell 
sẽ báo lỗi biên dịch trong tình huống tương tự. Ta phải 
minh bạch trong việc quy đổi kiểu bằng cách áp dụng hàm quy đổi.

Định kiểu mạnh đôi khi gây khó khăn trong việc viết những loại
mã lệnh nhất định. Chẳng hạn, một cách kinh điển để viết 
mã lệnh cấp độ thấp bằng ngôn ngữ C là với một mảng số kiểu byte
đã cho, ta quy đổi dể coi những số byte này như thể cấu trúc 
dữ liệu phức tạp. Cách này rất hiệu quả vì nó không yêu cầu ta phải
sao chép các byte qua lại. Hệ thống kiểu Haskell thì không cho phép
ta quy đổi vậy. Để có được cùng cái nhìn có cấu trúc về khối
dữ liệu này, ta phải sao chép, và như vậy sẽ tốn thêm chút
hiệu năng máy tính.

Lợi ích lớn của định kiểu mạnh là nó bắt được những lỗi thực sự nằm
trong mã lệnh trước khi chúng gây nên trục trặc. Chẳng hạn, trong
một ngôn ngữ định kiểu mạnh, ta không thể vô ý dùng một chuỗi 
khi chương trình trông đợi số nguyên.

#+BEGIN_NOTE
Các kiểu yếu và mạnh hơn

Cần nhận thấy rằng nhiều cộng đồng ngôn ngữ có định nghĩa riêng của họ
về một "kiểu mạnh". Tuy nhiên, chúng tôi sẽ nói vắn tắt và
bằng những thuật ngữ chung về khái niệm độ mạnh trong hệ thống
kiểu.

Giới khoa học máy tính coi ý nghĩa của "mạnh" và "yếu"
chỉ hạn hẹp về mặt kĩ thuật: mạnh là nói xem một hệ thống kiểu 
có /mức tùy tiện/ (permissive) đến đâu. Một hệ thống kiểu yếu 
sẽ coi những biểu thức là đúng thường xuyên hơn so với 
hệ thống kiểu mạnh.

Chẳng hạn, trong Perl, biểu thức ~"foo" + 2~ ước lượng thành
số 2, nhưng biểu thức ~"13foo" + 2~ lại ước lượng thành số
~15~. Haskell bác bỏ cả hai biểu thức này, cho rằng chúng đều 
không hợp lệ, vì toán tử ~(+)~ yêu cầu cả hai toán hạng đều
phải là số. Vì hệ thống kiểu Perl tùy tiện hơn của Haskell, nên
ta nói rằng hệ thống kiểu Perl yếu hơn theo nghĩa hẹp này.

Tranh cãi bùng nổ xoay quanh hệ thống kiểu đã bắt nguồn từ 
tiếng Anh, khi người ta gắn khái niệm về /giá trị/ vào các từ
"yếu" và "mạnh": ta thường nghĩ mạnh phải tốt hơn yếu. Lập trình 
viên nói tiếng Anh giao tiếp thì nhiều hơn là nói thứ tiếng
khoa học kĩ thuật, và nhiều lúc giới học thuật /thực sự/ đã
bắt bẻ tài nhẫn những hệ thống kiểu mà họ không ưa. Kết quả là
thường dẫn đến những cuộc cãi vã lê thê trên mạng, một kiểu
giải trí hay gặp.
#+END_NOTE

*** Kiểu tĩnh

Việc có một hệ thống kiểu /tĩnh/ đồng nghĩa với trình biên dịch
biết được kiểu của từng giá trị và biểu thức tại lúc biên dịch, 
trước khi thực thi mã lệnh. Trình biên dịch hoặc trình thông dịch
Haskell sẽ phát hiện khi ta cố thử dùng các biểu thức mà 
kiểu dữ liệu không khớp, và bác bỏ ngay mã lệnh có kèm theo một
lời thông báo lỗi trước khi chạy mã.
...
Những lời thông báo lỗi như thế này ta chưa từng gặp. Trình biên dịch
đã suy luận rằng kiểu của biểu thức ~"false"~ là [Char].
Toán tử ~(&&)~ yêu cầu từng toán hạng phải thuộc kiểu
~Bool~, và quả thực toán hạng bên trái của nó mang kiểu này. Vì 
thực ra kiểu ~"false"~ không khớp với kiểu được yêu cầu nên 
trình biên dịch đã bác bỏ biểu thức này với lí do sai kiểu.

Việc định kiểu tĩnh đôi khi có thể làm ta gặp khó khi viết mã lệnh
hữu ích. Ở những ngôn ngữ như Python, "định kiểu vịt" rất
thông dụng, trong đó đối tượng nào có biểu hiện đủ giống thứ
đang xét sẽ được dùng để thay thế nó[fn:1]. Thật may là
hệ thống /lớp kiểu/ của Haskell, mà ta sẽ đề cập trong 
[[file:6-using-typeclasses.org][Chương 6, Sử dụng lớp kiểu]], 
cho ta hầu hết những ưu điểm của định kiểu động, một cách
rất an toàn và tiện dụng. Haskell có hỗ trợ lập trình với 
kiểu động thực thụ, dù rằng không dễ như với một ngôn ngữ 
vốn thuần túy theo khái niệm kiểu động này.

Khả năng kết hợp định kiểu mạnh và tĩnh của Haskell giúp nó ngăn
không cho xảy ra lỗi kiểu lúc chạy chương trình. Dù điều này 
đồng nghĩa với việc ta phải nghĩ trước một số điều, nó cũng 
loại trừ nhiều lỗi đơn giản mà sẽ cực kì khó tìm nếu không dựa vào
tính năng này. Trong cộng đồng Haskell có một chân lí rằng một khi
mã lệnh đã biên dịch được thì nhiều khả năng chương trình sẽ chạy 
đúng hơn so với những ngôn ngữ khác. (Có lẽ một cách hiểu thực tế hơn
đó là mã lệnh Haskell thường có ít lỗi lặt vặt.)

Khi viết các chương trình bằng ngôn ngữ định kiểu động sẽ cần phải
có khối lượng kiểm thử rất đồ sộ để đảm bảo rằng những lỗi định kiểu
đơn giản không thể xảy ra. Bộ kiểm thử không bao quát hết được: 
một số nhiệm vụ thông thường như tái cấu trúc lại một chương trình
để nó được phân lập (modular) hơn, có thể sẽ đưa thêm những lỗi về kiểu
mới mà bộ kiểm thử không phát hiện ra.

Trong Haskell, trình biên dịch giúp ta chứng tỏ rằng các lỗi về kiểu
không tồn tại: một chương trình Haskell đã biên dịch thì sẽ không bị 
lỗi về kiểu khi chạy. Tái cấu trúc thường là việc thêm bớt mã lệnh, 
và rồi biên dịch lại cùng với dọn dẹp thêm vài lượt nữa đến khi
trình biên dịch báo cho ta "mọi thứ đều ổn".

Một hình ảnh tương đồng giúp ta hiểu được giá trị của định kiểu tĩnh 
là coi nó như việc đặt các miếng ghép trong trò chơi ghép hình. 
Với Haskell, nếu một miếng có hình sai thì đơn giản là nó sẽ không
cho phép đặt vào. Trong một ngôn ngữ định kiểu động thì mọi miếng ghép
đều là hình vuông 1x1 và sẽ luôn xếp khít nhau, vì vậy bạn phải thường
xuyên để ý hình được ghép thành và kiểm tra (qua việc kiểm thử)
xem hình đó đúng không.

*** Suy luận kiểu

Sau cùng, trình biên dịch Haskell có thể tự động suy luận kiểu của
hầu hết[fn:2] mọi biểu thức trong một chương trình. Quá trình này
được gọi là /suy luận kiểu/. Haskell cho phép ta khai báo minh bạch
kiểu của mọi giá trị, nhưng gần như mọi trường hợp bạn không cần
khai báo vì đã có suy luận kiểu rồi.
